...
class Blockchain:
    ...
    def add_transaction(self, json2):
        data1 = json2
    ...
    def announce_data_in_transaction_pool(self,new_transaction):
        peers = self.nodes
        for peer in peers:
            try:
                url = "http://{}/add_to_pool".format(peer)
                requests.post(url, json = json.dumps(new_transaction, sort_keys=True))
            except:
                print(f"Not all peers: {peer}")
        

# Creating Web App data=json.dumps(block, sort_keys = True)
...
def add_transaction():
    ...
    transaction_keys = ['sensors', 'factoryID', 'factoryName', 'timestamp']
    if not all (keys in json2 for keys in transaction_keys):
        return "Some elements of the transaction are missing" , 400
    index = blockchain.add_transaction(json2)
    blockchain.announce_data_in_transaction_pool(json2)
...
@app.route('/add_to_pool', methods= ['POST', 'GET'])
def add_to_pool():
    json2 = json.loads(request.get_json())
    transaction_keys = ['sensors', 'factoryID', 'factoryName', 'timestamp']
    if not all (keys in json2 for keys in transaction_keys):
        return "Some elements of the transaction are missing" , 400
    blockchain.add_transaction(json2)
    print("Added new transaction to pool")
    return "Added to transaction pool"
